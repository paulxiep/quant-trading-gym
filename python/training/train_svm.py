...
JSON uses "weights"/"biases" keys which map directly to the Rust
LinearPredictor's field names.
...
def train_svm_model(X_train, y_train, X_test, y_test, feature_names, config, name):
    """Train one LinearSVC model. Returns (clf, result_dict)."""
    print(f"\n=== Training LinearSVC: {name} ===")
    ...
    result = {
        ...
        "n_classes": 3,
        "classes": [-1, 0, 1],
        "weights": coef_eff.tolist(),
        "biases": intercept_eff.tolist(),
        "metadata": {
...
def train_all_svm(X_train, y_train, X_test, y_test, feature_names, config, output_dir):
    """Train all SVM models from config. Returns {rust_name: (clf, accuracy)}."""
    trained = {}
    for model_config in config.get("svm_models", []):
        name = model_config.get("name", "unnamed")
        clf, result = train_svm_model(
            X_train, y_train, X_test, y_test, feature_names, model_config, name
        )
        save_model_json(result, "svm_linear", name, output_dir)
        pkl_path = output_dir / f"{name}_svm_linear.pkl"
        with open(pkl_path, "wb") as f:
            pickle.dump(clf, f)
        rname = rust_model_name("svm_linear", name)
...
def main():
    """Standalone entry point for SVM training."""
    parser = argparse.ArgumentParser(description="Train LinearSVC models")
    ...
    train_all_svm(X_train, y_train, X_test, y_test, feature_names, config, output_dir)
